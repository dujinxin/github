...
@interface DJXAppDelegate ()<BMKGeneralDelegate>{
    NSInteger         selectTabBar;
}
...
//        [[UINavigationBar appearance] setTitleTextAttributes:attributes];
    ...
    BOOL ret = [_mapManager start:@"D9Pzp9cgOIUptunSnjHdMqGB" generalDelegate:self];
    if (!ret) {
        NSLog(@"BaiduManager start failed!");
    }
...
//    rootController.rightViewController = rightController;
    ...
    AppLaunchType type = kAppInstallFirst;
    if ([[NSUserDefaults standardUserDefaults] objectForKey:@"AppLaunchType"]){
        type = (AppLaunchType)[[[NSUserDefaults standardUserDefaults] objectForKey:@"AppLaunchType"] integerValue];
        if (type == kAppInstallFirst) {
            [SYAppStart show];
        }else if (type == kAppLanuchFirst){
...
- (NSArray *)getTabBarIconArray{
    NSDictionary *homeIconDic   = @{@"Normal": [UIImage imageNamed:@"tabBar_home_normal"],@"Selected" : [UIImage imageNamed:@"tabBar_home_press"],@"Title" : @"首页"};
    NSDictionary *shoppingIconDic = @{ @"Normal" : [UIImage imageNamed:@"tabBar_category_normal"],@"Selected" : [UIImage imageNamed:@"tabBar_category_press"],@"Title" : @"分类" };
    NSDictionary *shakeIconDic = @{ @"Normal" : [UIImage imageNamed:@"tabBar_find_normal"],@"Selected" : [UIImage imageNamed:@"tabBar_find_press"],@"Title" : @"发现" };
    NSDictionary *likeIconDic = @{ @"Normal" : [UIImage imageNamed:@"tabBar_myJD_normal"],@"Selected" : [UIImage imageNamed:@"tabBar_myJD_press"],@"Title" : @"我的" };
    NSDictionary *myIconDic = @{ @"Normal" : [UIImage imageNamed:@"tabBar_cart_normal"],@"Selected" : [UIImage imageNamed:@"tabBar_cart_press"],@"Title" : @"购物车" };
...
- (BOOL)tabBarController:(LeveyTabBarController *)tabBarController shouldSelectViewController:(UIViewController *)viewController
{
    ...
    if (index == 4)
    {
        ...
        __block DJXAppDelegate *weakSelf = self;
            cartViewCon.callBackBlock = ^(){
                [weakSelf.mytabBarController setSelectedIndex:weakSelf->selectTabBar];
            };
...
- (void)onGetNetworkState:(int)iError{
    if (0 == iError) {
        ...
        NSLog(@"baiduLocationNetworkError:%d",iError);
    }
}
...
- (void)onGetPermissionState:(int)iError{
    if (0 == iError) {
        ...
        NSLog(@"baiduLocationPermissionError:%d",iError);
    }
}
@end
